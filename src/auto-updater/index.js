...
const nutsURL =
  "https://api.update.rocks/update/github.com/Nitingadhiya/test/stable";

if (os.platform() === "darwin") {
  updateFeed = `${nutsURL}/${platform}/${appVersion}`;
} else if (os.platform() === "win32") {
  updateFeed = `${nutsURL}/win32/${appVersion}`;
}
...
function init(mainWindow) {
  ...
  autoUpdater.once("update-downloaded", (ev, err) => {
    const msg =
      '<p style="margin: 0;">🤘 Update downloaded - <a onclick="quitAndInstall()">Restart</a></p>';
    mainWindow.webContents.send("message", {
      msg,
      hide: false,
      replaceAll: true
